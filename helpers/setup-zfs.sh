...
if ! command -v zfs &>/dev/null || ! command -v zpool &>/dev/null; then
    ...
    if [[ "$inst_zfs" =~ ^[Yy]$ ]]; then
        echo "--> Updating repository index and installing ZFS packages..."
        apt-get update
        apt-get install -y linux-headers-amd64 zfs-dkms zfsutils-linux
        
        # Load ZFS kernel module
        echo "--> Loading ZFS kernel module..."
        modprobe zfs || { echo -e "${RED}Error loading ZFS module. You may need to reboot first.${NC}"; exit 1; }
    else
        echo -e "${RED}Aborted: ZFS setup cannot proceed without ZFS utilities.${NC}"
        exit 1
    fi
fi
